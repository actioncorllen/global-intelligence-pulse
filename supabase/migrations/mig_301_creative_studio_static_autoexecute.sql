-- STRATELOQ — auto-trigger the Creative Image Executor after STATIC dispatch
-- ============================================================================
-- Closes the automatic loop: when fn_creative_studio_generate moves a STATIC image
-- job to GENERATING, it fires (pg_net, server-side) the creative-image-execute edge
-- function, which resolves+claims the job, runs the OpenAI edit via the n8n executor,
-- uploads the asset, and calls fn_media_complete_image_real. The browser never calls
-- n8n and never receives service-role. The edge gateway apikey used here is the PUBLIC
-- anon publishable key (not a secret); tenant safety is enforced by the job's idempotent
-- claim (fn_media_image_execution_context), so re-fires from polling are absorbed
-- (already_claimed -> skip). VIDEO is untouched (paid Gemini/Veo stays gated).
-- ============================================================================

CREATE OR REPLACE FUNCTION public.fn_creative_fire_image_executor(p_job_id uuid)
 RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO ''
AS $function$
BEGIN
  PERFORM net.http_post(
    url := 'https://nxaunmyihhjixxxljcqt.supabase.co/functions/v1/creative-image-execute',
    body := jsonb_build_object('job_id', p_job_id),
    headers := jsonb_build_object(
      'Content-Type','application/json',
      'apikey','eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im54YXVubXlpaGhqaXh4eGxqY3F0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI2Nzk1ODcsImV4cCI6MjA5ODI1NTU4N30.K17MFaFZSTrJwzHetNijeA188GaxvEXxwgZWIrSLBoQ',
      'Authorization','Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im54YXVubXlpaGhqaXh4eGxqY3F0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI2Nzk1ODcsImV4cCI6MjA5ODI1NTU4N30.K17MFaFZSTrJwzHetNijeA188GaxvEXxwgZWIrSLBoQ'
    )
  );
END; $function$;
REVOKE EXECUTE ON FUNCTION public.fn_creative_fire_image_executor(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fn_creative_fire_image_executor(uuid) TO service_role;
...
BEGIN
  ...
  RETURN jsonb_build_object('ok',true,'request_id',p_request_id,'creative_format',r.creative_format,
    ...
    'note', CASE WHEN v_output='STATIC'
      THEN 'Static creative dispatched and the executor was triggered automatically; job is GENERATING and completes via the Creative Image Executor.'
